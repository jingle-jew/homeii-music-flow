...
export function queueItemMatchTitle(item = {}) {
  return normalizeComparableText(item?.media_item?.name || item?.media_title || item?.name || "");
}

export function queueItemMatchArtist(item = {}) {
...
export function queueTitleArtistMatch(item, title = "", artist = "") {
...
export function findQueueItemIndex(queueItems = [], {
  queueItemId = "",
  uri = "",
  sortIndex = "",
  fallbackType = "track",
